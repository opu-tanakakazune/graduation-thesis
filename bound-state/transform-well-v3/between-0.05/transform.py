...
calculate_s = 0
#  t=0のdataファイル作成
with open('bound-state/transform-well-v3/between-0.05/schrodingerV.dat', 'w') as f1:
    for i in range(n):
        x = -5.0 + h*i
        expected_value = (abs(old_phi[i]))**2
        calculate_s += expected_value*h
        print(x, expected_value, v[i]/20 + constant_v/20, file=f1)
    s += [calculate_s]
    print(s)
...
for m in range(2,25):
    file_name = 'bound-state/transform-well-v3/between-0.05/schrodinger' + str(m) + '.dat'
    f_number = 'f' + str(m)
    ...
    with open(file_name, 'w') as f_number:
        for i2 in range(n):
            ...
            calculate_s += expected_value*h
            print(x, expected_value, v[i2]/10, file=f_number)
    s += [calculate_s]
    print(s)
