...
for i in range(n):
    ...
    if -l <= x and x <= l:
        phi_value = C * cmath.cos(k * x) + 0j
    else:
        phi_value = C * cmath.cos(k * l) * cmath.exp(-1 * (ka * (abs(x) - l))) + 0j
    v_value = -constant_v
    if -l - 0.05 <= x and x < -l:
        v_value = 0
    if l < x and x <= l + 0.05:
...
calculate_s = 0
#  t=0のdataファイル作成
with open('bound-state/transform-well/schrodinger1.dat', 'w') as f1:
    for i in range(n):
        x = -5.0 + h*i
...
for m in range(2,3):
    ...
    with open(file_name, 'w') as f_number:
        for i2 in range(n):
            ...
            calculate_s += expected_value*h
            print(x, expected_value, v[i2], file=f_number)
    s += [calculate_s]
    print(s)
